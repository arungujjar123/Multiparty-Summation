  ...
  const loadAchievements = useCallback(() => {
    ...
    let currentAchievements = ACHIEVEMENTS_DATA.map((achievement) => {
      ...
      return {
        ...
        unlockedDate: isPersistentUnlocked
          ? unlockedAchievements[achievement.id].unlockedDate
          : undefined,
      };
    });
    ...
    if (typeof window !== "undefined") {
      const achievementsToSave = currentAchievements.reduce(
        (acc, a) => {
      ...
      <div className="max-w-7xl mx-auto px-6 py-12">
          ...
          <div className="mb-12">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {unlocked.map((a) => (
                <div
                  key={a.id}
                  className="p-6 bg-gradient-to-br from-yellow-50 to-orange-50 dark:from-yellow-900/20 dark:to-orange-900/20 rounded-xl shadow-lg border-2 border-yellow-400 hover:shadow-2xl transition-all transform hover:scale-105 animate-fade-in"
                >
                  ...
                  {a.unlockedDate && (
                    <div className="text-xs text-gray-500">
                      Unlocked: {new Date(a.unlockedDate).toLocaleDateString()}
                    </div>
                  )}
                </div>
              ))}
          ...
          <div>
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {locked.map((a) => (
                <div
                  key={a.id}
                  className="p-6 bg-white dark:bg-gray-800 rounded-xl shadow-lg border-2 border-gray-300 dark:border-gray-700 opacity-60 hover:opacity-80 transition-all"
                >
                  <div className="flex justify-between mb-4">
        ...
        <div className="mt-12 text-center">
          <button
            ...
            className="px-6 py-3 bg-red-600 hover:bg-red-700 text-white font-semibold rounded-lg shadow-lg transition-all transform hover:scale-105"
          >
            🔄 Reset All Progress
          </button>
        </div>
